...
export default function GoodsPage() {
  ...
  const displayedGoods = usesClientPagination
    ? orderedGoods.slice((filters.page - 1) * GOODS_PER_PAGE, filters.page * GOODS_PER_PAGE)
    : orderedGoods;
  ...
  const selectedSortLabel = SORT_OPTIONS.find((option) => option.value === filters.sort)?.label;
      ...
      try {
        ...
        if (usesClientPagination) {
          ...
          const remainingGoods = await Promise.all(
            remainingPages.map((page) => getGoods({
              subCategory: filters.subCategory,
              sort: filters.sort,
              page,
              limit: GOODS_PER_PAGE,
          ...
          setGoods(allGoods);
          setTotalPages(Math.max(1, Math.ceil(allGoods.length / GOODS_PER_PAGE)));
        } else {
          setGoods(data?.goods ?? []);
      ...
      <section aria-label="굿즈 목록">
        ...
        {loading ? (
          ...
          <div className="goods-page__grid">
            ...
            {Array.from(
              {
                length: (GRID_COLUMNS - (displayedGoods.length % GRID_COLUMNS)) % GRID_COLUMNS,
              },
